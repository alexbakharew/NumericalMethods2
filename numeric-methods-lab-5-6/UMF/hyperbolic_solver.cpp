...
void HyperbolicSolver::ExplicitSolve() const
{
    ...
    for(int i = K - 3; i >= 0; --i)
    {
        for(int j = 1; j < N - 1; ++j)
        {
            mesh[i][j] = (sigma * (mesh[i + 1][j + 1] - 2 * mesh[i + 1][j] + mesh[i + 1][j - 1])) - (mesh[i + 1][j] * ((3 * tau * tau) - 2)) - mesh[i + 2][j];
        }
    }
}
...
void HyperbolicSolver::ImplicitSolve() const
{
    ...
    for(int i = K - 3; i >= 0; --i)
    {
        ...
        for(int j = 0; j < N; ++j)
        {
            auto tmp = (((3 * tau * tau) - 2) * mesh[i + 1][j]) + mesh[i + 2][j];
            if(j == 0)
            {
                matrix[0][0] = b;
                matrix[0][1] = c;
                res[0] = tmp - sigma * boundary_condition_x0(i * tau);
            }
            else if(j == N - 1)
            {
                matrix[N - 1][N - 2] = a;
                matrix[N - 1][N - 1] = b;
                res[N - 1] = tmp - sigma * boundary_condition_xl(i * tau);
            }
            else
            {
                matrix[j][j - 1] = a;
